...
def get_gene_length(GTFfile,genelength_file='',chroms=map(str,range(1,23))+['X','Y']):
	...
	ret_isoform_length = get_isoform_length(GTFfile,chroms=chroms) #bug here
...
parser.add_argument('-v','--version',action='version',version="GTFtools version:0.6.5jy")
...
#changed here, do nothing for format convertion
#if ftype == 'GENCODE':
...
#changed here to add whatever genomes
...
if args.chroms:
	...
	chromsargs='grep -v \"#\" '+GTFfile+" | cut -f 1 | sort | uniq"
	#print chromsargs
	with os.popen(chromsargs) as pse:
		for line in pse:
			line=line.rstrip("\r\n")
			#print line
			chroms.append(line)
