...
convert_chipseq_de_to_bed
version: $version
Usage: perl  [parameters]

Description: convert chipseq de peaks to bed for tfbs analysis


Mandatory Parameters:
    --in|-i           Input file from chipseq-de
    --out|--o         Output file, bed files for up,down,both
		...
		unless(defined $fccutoff && length($fccutoff)>0 && defined $qcutoff && length($qcutoff)>0) {
			if($array[5] eq "1" || $array[5] eq "-1") {	
				if($array[5] eq "1") {
					...
					print OUT1 $1,"\t",$2-1,"\t",$3,"\t",$array[0],"\n";
				}
				else {
					#print OUT2 $array[0],"\t",$1,"\t",$2,"\t",$3,"\t",".","\n";
					print OUT2 $1,"\t",$2-1,"\t",$3,"\t",$array[0],"\n";
				}
				#print OUT $array[0],"\t",$1,"\t",$2,"\t",$3,"\t",".","\n";
				print OUT $1,"\t",$2-1,"\t",$3,"\t",$array[0],"\n";
			}
		}
		else {
